...
select
   -- identifiers
    ...
    -- date filters
    data_file_year as trip_year,
    data_file_month as trip_month,
    case 
        when data_file_month > 9 then "Q4"
        when data_file_month > 6 then "Q3"
        when data_file_month > 3 then "Q2"
    ELSE "Q1"
    END as trip_quarter,
    case 
        when data_file_month > 9 then concat(data_file_year,"-","Q4")
        when data_file_month > 6 then concat(data_file_year,"-","Q3")
        when data_file_month > 3 then concat(data_file_year,"-","Q2")
    ELSE concat(data_file_year,"-","Q1")
    END as year_quarter,
